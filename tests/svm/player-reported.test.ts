...
function makeTournament(o: {
  ...
  return {
    ...
    disputeWindowSecs: o.disputeWindowSecs,
...
function decode(
  svm: LiteSvm,
  name: "tournament" | "matchNode" | "participant",
  pubkey: PublicKey
): any {
  const acc = svm.getAccount(pubkey.toBytes())!;
...
async function seedTournament(
  ...
  await writeAccount(
    ...
    makeTournament({
      ...
      disputeWindowSecs: o.disputeWindowSecs,
      bracketSize: o.bracketSize,
      participantCount: o.bracketSize,
      matchesInitialized: o.bracketSize - 1,
      matchesReported: 0,
      totalMatches: o.bracketSize - 1,
    })
  );
...
async function seedMatch(
  ...
  await writeAccount(
    ...
    pA,
    "participant",
    makeParticipant({ tournament, wallet: playerA, bump: pABump })
  );
  await writeAccount(
    svm,
    pB,
    "participant",
    makeParticipant({ tournament, wallet: playerB, bump: pBBump })
  );
  return { match, participantA: pA, participantB: pB };
}
async function seedEmptyMatch(
  svm: LiteSvm,
  tournament: PublicKey,
  round: number,
  idx: number
  ...
  await writeAccount(
    ...
    makeMatch({
      ...
      round,
      matchIndex: idx,
      playerA: PublicKey.default,
      playerB: PublicKey.default,
      bump: mBump,
      status: { pending: {} },
    })
  );
...
describe("player-reported settlement (LiteSVM)", function () {
  ...
  it("propose+confirm advances the bracket and credits stats (non-final)", async () => {
    const svm = bootSvm();
    const b = await seedTournament(svm, {
      name: "pr-happy",
      settlementMode: { playerReported: {} },
      disputeWindowSecs: 60,
      bracketSize: 4,
      vaultAmount: 4n * ENTRY_FEE,
    });
    const A = funded(svm),
      B = funded(svm);
    const {
      ...
      participantA,
      participantB,
    } = await seedMatch(svm, b.tournament, 0, 0, A.publicKey, B.publicKey);
    const final = await seedEmptyMatch(svm, b.tournament, 1, 0);

    expectOk(
      sendTx(svm, proposeIx(A.publicKey, b.tournament, semi, A.publicKey), A)
    );
    expectOk(
      sendTx(
        svm,
    ...
    expect(decode(svm, "matchNode", final).playerA.toBase58()).to.equal(
  ...
  it("confirm of the final distributes the prize pool", async () => {
    ...
    const b = await seedTournament(svm, {
      ...
      disputeWindowSecs: 60,
      bracketSize: 2,
      vaultAmount: 2n * ENTRY_FEE,
    });
    const champ = funded(svm),
      loser = funded(svm);
    const {
      match: final,
      participantA,
    ...
    expectOk(
      sendTx(
        svm,
        proposeIx(champ.publicKey, b.tournament, final, champ.publicKey),
        champ
      )
    );
    ...
    expectOk(
      sendTx(
        svm,
        buildIx("confirmResult", keys, { placements: [champ.publicKey] }),
        loser
      )
    );
  ...
  it("rejects propose on OrganizerOnly tournaments (SettlementModeMismatch)", async () => {
    ...
    const A = funded(svm),
      B = funded(svm);
    const { match: semi } = await seedMatch(
      svm,
      b.tournament,
      0,
      0,
