...
function makeTournament(o: {
  organizer: PublicKey; arbitrator: PublicKey; name: string; vault: PublicKey;
  bump: number; vaultBump: number; payoutPreset: any;
}) {
  return {
    ...
    organizerDepositRefunded: false, maxParticipants: 4, bracketSize: 2,
    participantCount: 2, matchesInitialized: 1, matchesReported: 0, totalMatches: 1,
    registrationDeadline: new BN(0), createdAt: new BN(0), startedAt: new BN(1_000_000),
    completedAt: new BN(0), status: { active: {} }, payoutPreset: o.payoutPreset,
    seedHash: Array(32).fill(0), champion: PublicKey.default, bump: o.bump, vaultBump: o.vaultBump,
    game: { manual: {} }, settlementMode: { playerReported: {} }, disputeWindowSecs: 0,
...
function makeMatch(o: { tournament: PublicKey; playerA: PublicKey; playerB: PublicKey; bump: number }) {
  return {
    tournament: o.tournament, bracket: 0, round: 0, matchIndex: 0,
    playerA: o.playerA, playerB: o.playerB, winner: PublicKey.default,
    status: { active: {} }, bye: false, bump: o.bump,
...
function decode(svm: LiteSvm, name: "tournament" | "matchNode", pubkey: PublicKey): any {
  const acc = svm.getAccount(pubkey.toBytes())!;
...
async function seedFinal(svm: LiteSvm, name: string, payoutPreset: any) {
  ...
  await writeAccount(svm, tournament, "tournament", makeTournament({
    organizer: organizer.publicKey, arbitrator: organizer.publicKey, name, vault, bump, vaultBump, payoutPreset,
  }));
  await writeAccount(svm, final, "matchNode", makeMatch({ tournament, playerA: champion.publicKey, playerB: runnerUp.publicKey, bump: mBump }));
  await writeAccount(svm, pA, "participant", makeParticipant({ tournament, wallet: champion.publicKey, bump: pABump }));
  await writeAccount(svm, pB, "participant", makeParticipant({ tournament, wallet: runnerUp.publicKey, bump: pBBump }));
  ...
  expectOk(sendTx(svm, buildIx("proposeResult", [
    meta(champion.publicKey, true, true), meta(tournament, false, false), meta(final, false, true),
  ], { proposedWinner: champion.publicKey }), champion));

  return { organizer, treasury, champion, runnerUp, tournament, vault, pcfg, final,
    participantA: pA, participantB: pB, treasuryAta, champAta, runnerAta };
}
...
describe("settle_final / H-1 multi-placement final hardening (LiteSVM)", function () {
  it("blocks a permissionless multi-placement final, then settles it via the arbitrator", async () => {
    ...
    const stolen = [...finalizeKeys(cranker.publicKey, ctx),
      meta(ctx.champAta, false, true), meta(ctx.runnerAta, false, true),
      meta(attackerAta, false, true), meta(ctx.treasuryAta, false, true)];
    expectCustomErr(
      sendTx(svm, buildIx("claimResult", stolen, { placements: [champion, runnerUp, attacker.publicKey] }), cranker),
    ...
    const correct = [
      meta(ctx.champAta, false, true), meta(ctx.runnerAta, false, true),
      meta(thirdAta, false, true), meta(ctx.treasuryAta, false, true),
    ];
    expectCustomErr(
      sendTx(svm, buildIx("settleFinal", [...finalizeKeys(attacker.publicKey, ctx), ...correct], { placements: [champion, runnerUp, third] }), attacker),
      ERR_UNAUTHORIZED,
    );
    ...
    expectOk(
      sendTx(svm, buildIx("settleFinal", [...finalizeKeys(ctx.organizer.publicKey, ctx), ...correct], { placements: [champion, runnerUp, third] }), ctx.organizer),
    );
    ...
    expect(Object.keys(decode(svm, "matchNode", ctx.final).status)[0]).to.equal("completed");
  });
  ...
  it("still lets anyone permissionlessly claim a WinnerTakesAll final", async () => {
    ...
    const cranker = funded(svm);
    const keys = [...finalizeKeys(cranker.publicKey, ctx),
      meta(ctx.champAta, false, true), meta(ctx.treasuryAta, false, true)];
    expectOk(sendTx(svm, buildIx("claimResult", keys, { placements: [champion] }), cranker));

    const net = GROSS - (GROSS * FEE_BPS) / 10_000n;
    expect(tokenAmount(svm, ctx.champAta)).to.equal(net);
    expect(Object.keys(decode(svm, "tournament", ctx.tournament).status)[0]).to.equal("completed");
  });
});
