...
The SDK + Indexer consume typed accounts/instructions generated from `target/idl/bracket_chain.json` by [`@codama/cli`](https://github.com/codama-idl/codama) using `@codama/renderers-js@2.x`. Configuration lives at `codama.json`. The renderer's default package-wrapped layout is overridden to write a flat tree directly into each consumer's `src/generated/` (`generatedFolder: "."`, `syncPackageJson: false`), so codama overwrites existing generated files in place instead of nesting them under `src/generated/src/generated/`. For the Indexer, `deleteFolderBeforeRendering: false` is also set so the sibling `src/generated/prisma/` tree (regenerated by Prisma, not Codama) is preserved between runs.
